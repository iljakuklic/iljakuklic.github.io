...
posuere. Fusce pellentesque gravida tempus. $a^2+b^2=c^2$ Integer viverra tortor nec eros
...
malesuada ut. Donec suscipit elit id leo vehicula mattis non sed leo.^[this is a footnote]

WTF diff `a -> Maybe a`{.lang-haskell}.

~~~~ lang-haskell
{-# LANGUAGE NoMonomorphismRestriction #-}
{- Regular comment HERE   foo bar baz  -}

import Data.Monoid

data Foo a = Yea a | Nope

mapMaybe :: (a -> b) -> (Maybe a -> Maybe b)
mapMaybe = (<$>)

myFunc = unsafePerformIO explode
...
![Haskell](/images/haskell-logo.png "Fooo")
...
~~~ lang-java
public static abstract interface void main(string args[]) {
    int x = 7 + 7;
    int y = x * x;
    for () {}
    return;
...
$Pair^n(T) = T^{2^n}$ for $n\in \mathbb{N}$
